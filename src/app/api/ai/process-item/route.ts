...
async function callOpenAI(input: string) {
  // ✅ Billing yokken geliştirme: mock çıktı üret
  if (process.env.AI_ENABLED !== "true") {
    return {
      title: "Saved research (mock)",
      summary:
        "AI is disabled. Set AI_ENABLED=true and add OPENAI_API_KEY to generate real summary/tags.",
      tags: ["research", "freelance", "clario"],
      category: "other",
    } as {
      title?: string;
      summary?: string;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // tags boşsa AI tags’i tags kolonuna da bas (UX için)
    ...
    // failed set (req.json tekrar okumuyoruz)
    if (itemId) {
      try {
        await supabaseAdmin
          .from("items")
